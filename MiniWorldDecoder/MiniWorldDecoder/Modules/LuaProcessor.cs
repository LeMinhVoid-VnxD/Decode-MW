...
public class LuaProcessor
{
    ...
    public DecodeResult Decode(string filePath, string outputDir, string? xxteaKey)
    {
        ...
        try
        {
            ...
            var data = TryDecodeBase64Data(raw);
            if (data != raw)
    ...
    private static byte[] TryDecodeBase64Data(byte[] raw)
    {
        var text = System.Text.Encoding.UTF8.GetString(raw);

        // Trim BOM/null/whitespace
        text = text.Trim('\0', '\uFEFF', ' ', '\t', '\r', '\n');

        // Check for "a0817i" prefix (MiniWorld encrypted signature)
        ...
        // Check if entire content looks like Base64 (no whitespace except at ends)
        if (text.Length >= 64 && text.All(c => char.IsAsciiLetterOrDigit(c) || c == '+' || c == '/' || c == '='))
        {
